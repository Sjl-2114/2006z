<template>
  <div>
    ...
    <br>
    <input type="text" v-model="keyword" placeholder="请输入搜索的内容">
    <br />
    <table style="width: 100%" class="table">
      ...
      <tbody>
        <tr v-for="(item, index) in lists" :key="index" :class="{backcolor:index%2}">
          <td>
            <input
              type="checkbox"
              :value="index"
          ...
          let goods = this.goods
            if (this.is !=0) {
             goods = goods.sort((a,b) =>{
                return this.is ==2 ? b.age - a.age: a.age - b.age;
            })
            } 
         goods =  goods.filter(item=>{
                return item.username.indexOf(this.keyword) >  -1
                return item.ss.indexOf(this.keyword) >  -1
            })
